      ...
      <div class="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-4 py-12 lg:flex-row lg:items-center lg:gap-12">
        <div class="mb-10 max-w-xl lg:mb-0">
          ...
          <p class="mt-4 text-lg font-normal text-mb-text-secondary">
            Internal dashboard for four branches — mock data today, your Nest API tomorrow.
          </p>
          <ul class="mt-8 space-y-3 text-sm text-mb-text-secondary">
            <li class="flex gap-2">
              ...
              Owner, manager, accountant, and barber permissions match a real internal ops model.
            </li>
            <li class="flex gap-2">
              <span class="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-mb-primary"></span>
              Try dark mode after sign-in — use the theme button in the header.
            </li>
          </ul>
        </div>
        ...
        <div class="w-full max-w-md space-y-6">
          ...
          <mb-card title="Demo accounts" subtitle="One tap fills the form">
              ...
              @for (u of demoUsers; track u.email) {
                ...
                >
                  ...
                  <span
                    ...
                    >{{ u.email }}</span
                  >
                  <span class="mt-1 inline-block rounded-md bg-emerald-500/15 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-emerald-800 dark:text-emerald-300">
                    {{ roleLabel(u) }}
                  </span>
                </button>
              }
  ...
  submit(): void {
    ...
    this.auth.login(email, password).subscribe({
      ...
      error: () => {
        ...
        this.error.set('Invalid email or password (use demo list).');
      },
    });
  }
}
