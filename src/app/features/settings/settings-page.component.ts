  ...
  template: `
    <div class="mx-auto max-w-2xl space-y-6 md:space-y-8 lg:space-y-10">
      <div class="mb-page-header">
        <h1 class="mb-page-title">Settings</h1>
        <p class="mb-page-sub">Session · mock preferences only</p>
      </div>
      ...
      @if (auth.canManageBusiness()) {
        <mb-card title="Workspace branding" subtitle="Logo in the sidebar, header, and account menu (stored locally for demo)">
          <div class="flex flex-col gap-5 sm:flex-row sm:items-start">
            <div class="flex shrink-0 flex-col items-center gap-2">
              ...
              @if (branding.logoDataUrl()) {
                <mb-btn variant="ghost" size="sm" (click)="clearLogo()">Remove logo</mb-btn>
              }
            </div>
            <div class="min-w-0 flex-1 space-y-3">
              <div>
                ...
                <input
                  type="text"
                  class="mb-input"
                  placeholder="Mubase Saloon"
                  [value]="branding.businessName() ?? ''"
                  (input)="onBusinessNameInput($event)"
                />
                <p class="mt-1.5 text-xs text-mb-text-secondary">
                  Shown as the main title for owners and in the account menu.
                </p>
              </div>
            </div>
          </div>
...
export class SettingsPageComponent {
  ...
  onLogoFile(ev: Event): void {
    ...
    if (file.size > 450_000) {
      window.alert('Please choose an image under 450 KB (demo limit).');
      return;
    }
